...
import styles from './VistaPreviaCSS.module.css';
            ...
            >
                ...
                <div className={styles.carouselImages}>
                    <Swiper spaceBetween={10} slidesPerView={2} loop={true}
                        ...
                        modules={[Autoplay]}
                    >
                        {contenido?.imagenesCarrousel?.length > 0 ? (
                            contenido?.imagenesCarrousel?.map((item, index) => (
                                <SwiperSlide key={index} className={styles.carouselItem}>
                                    <img src={item.imagen} alt={`Carrousel ${index}`} className={styles.imgCarrousel} />
                                    <h3>{item.nombre}</h3>
                            ...
                            [1, 2, 3, 4, 5, 6].map((_, index) => (
                                <SwiperSlide key={index}>
                                    <Skeleton variant="rectangular" width={210} height={118} sx={{ cursor: 'grab' }} />
                                </SwiperSlide>
                            ))
                        )}
                    </Swiper>
